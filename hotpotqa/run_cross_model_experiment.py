#!/usr/bin/env python3
"""
Cross-model validation: 20 questions x 10 runs.
Runs inside a k8s pod alongside the model server sidecar.
Identical to run_60q_experiment.py but for 20-question subset.

Usage:
    python run_cross_model_experiment.py --questions /data/cross_model_20q.json --output /results/qwen_cross_model
    python run_cross_model_experiment.py --questions /data/cross_model_20q.json --output /results/qwen_cross_model --resume
"""
...
async def run_experiment(questions_path: str, output_dir_str: str, resume: bool):
    ...
    logger.info(f"Cross-model experiment: {n_questions} questions x {N_RUNS_PER_QUESTION} runs = {total_target} total")
    ...
    logger.info("=" * 70)
    logger.info("CROSS-MODEL EXPERIMENT COMPLETE")
...
def main():
    parser = argparse.ArgumentParser(description="Cross-model 20-question experiment")
