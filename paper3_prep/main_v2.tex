...
\title{Representational Commitment: \\ Hidden-State Convergence Predicts and Causes Agent Consistency}
...
\begin{abstract}
LLM-based agents produce different reasoning trajectories across runs of the same task---a critical reliability failure with no established internal explanation. We show that this behavioral inconsistency is predicted by a simple geometric property of hidden states. Using Llama-3.1-70B on ReAct question-answering (100 questions $\times$ 10 runs), we find that \emph{activation similarity}---pairwise cosine similarity of hidden states across runs at a given step---predicts downstream behavioral consistency. We call this \textbf{representational commitment}: when hidden states \emph{converge} across runs despite different observations, the model has arrived at a robust interpretation and behavior is consistent; when states \emph{diverge}, the model remains uncommitted and behavior scatters. The signal is temporally precise (absent steps 1--2, peaks step~4, fades step~5), with effect sizes of $r = -0.35$ (partial $r = -0.45$ after difficulty controls, $p < 10^{-5}$; 4.1$\times$ quartile effect, $d = 1.01$). It predicts \emph{process consistency} but not \emph{per-run correctness}, and generalizes across three model families (Llama~70B, Qwen~72B, Phi-3~14B) and two benchmarks, replicating strongly on StrategyQA ($r = -0.83$), a structurally different task with shorter reasoning chains. A runtime monitor using step-4 hidden states predicts which questions will produce inconsistent behavior (AUROC 0.97). A prompting intervention closes the causal loop: inducing commitment at step~3 increases hidden-state convergence at the commitment juncture ($d = 0.97$, $p < .001$) and reduces behavioral variance by 28\% ($d = 0.33$, $p = .001$) relative to a token-matched filler control, without affecting accuracy. Representational commitment provides both a diagnostic for agent reliability and a mechanistic account of why agent behavior is sometimes consistent and sometimes not.
\end{abstract}
...
We answer affirmatively. Extracting hidden states from Llama-3.1-70B \citep{llama3} during ReAct \citep{react} question-answering on HotpotQA \citep{hotpotqa}, we find that cross-run \emph{activation similarity}---how similar hidden states are across multiple runs of the same question---predicts downstream behavioral consistency. We introduce the concept of \textbf{representational commitment}: when a model's hidden states converge across runs despite receiving different observations, the model has robustly committed to a stable interpretation and behavior is consistent; when hidden states diverge across runs, the model's representation is sensitive to the specific evidence path and behavior scatters.

Our contributions:
\begin{enumerate}
    \item Activation similarity at step~4 predicts consistency ($r = -0.35$, permutation $p < 0.001$, $n{=}100$), \emph{strengthening} after difficulty controls (partial $r = -0.45$, $p < 10^{-5}$), with a 4.1$\times$ quartile effect ($d = 1.01$). The signal has temporal and spatial specificity: absent at steps 1--2, peaking at step~4, spanning layers 32--80.
    ...
    \item Cross-model validation on Qwen~2.5~72B ($r = -0.65$) and Phi-3-Medium-14B ($r = -0.36$), spanning 14B--72B parameters and three architectures, and cross-benchmark validation on StrategyQA ($r = -0.83$, $n{=}50$) confirm generalization across model families, scales, and task types. The pattern replicates at different peak layers across models, ruling out step/layer-specific selection artifacts.
    \item A consistency predictor using step-4 hidden states achieves AUROC up to 0.97 (Llama) and 0.93 (Qwen) via LOOCV, substantially outperforming surface-level baselines.
    \item A three-condition prompting intervention (control, token-matched filler, commitment) isolates the commitment framing effect: the commitment prompt reduces both step-count CV (28\%, $d = 0.33$, $p = .001$) and action sequence diversity (24\%, $d = 0.47$, $p < .001$) relative to the filler control, without affecting accuracy.
\end{enumerate}
...
\textbf{Why does high similarity predict consistency?} By step~4, each run has received different observations (search results, retrieved documents). When hidden states nonetheless \emph{converge} across runs (high similarity), the model has robustly arrived at a stable interpretation regardless of which specific evidence path it took---it has committed. Behavior from this committed state is consistent. When hidden states \emph{diverge} (low similarity), the model's representation is sensitive to the specific observations each run received---it remains uncommitted---and downstream behavior scatters.
...
The filler control does not reduce CV relative to control ($p = .071$), confirming that additional tokens alone do not improve consistency. The commitment prompt reduces CV by 28\% relative to filler (paired $t(99) = 3.29$, $p = .001$, $d = 0.33$; Wilcoxon $p < .001$), establishing that commitment \emph{framing}---not token count---drives the effect. Action diversity shows the same pattern (24\% reduction, $d = 0.47$, $p < .001$). No condition affects accuracy (all pairwise $p > .05$; the control-vs-commitment comparison is marginal at $p = .083$, but the critical filler-vs-commitment test---which isolates the commitment framing from token-count effects---shows no difference, $p = .32$). This is predicted by the correctness-agnostic nature of commitment (Section~4.4): inducing commitment makes the model adhere more firmly to whatever interpretation it has formed---correct or incorrect---so questions where the model was mostly right become more consistently right, while questions where it was mostly wrong become more consistently wrong, and these effects cancel in aggregate accuracy.

\textbf{Representational-level evidence.} We extracted hidden states across all three conditions and computed activation similarity at step~4 for $n{=}89$ matched questions (11 questions excluded because at least one condition had fewer than 2 runs reaching step~4 with complete hidden-state extraction; excluded questions do not differ systematically in accuracy or CV from the matched set). Figure~\ref{fig:intervention_step_progression} shows the critical result: all three conditions produce \emph{identical} similarity trajectories through step~3 (the intervention point), then diverge sharply at step~4. The commitment condition produces the highest similarity (0.995), followed by filler (0.979) and control (0.922). Crucially, the commitment--filler comparison---controlling for shared text---shows the commitment framing specifically increases representational convergence beyond the trivial effect of shared tokens ($\Delta = +0.016$, $d = 0.97$, $p < .001$; commitment vs.\ control: $\Delta = +0.075$, $d = 1.14$, $p < .001$; all at layer~40). The filler's intermediate similarity (0.979) reflects lexical overlap from the shared prompt text---a superficial convergence that does not constitute representational commitment, as evidenced by the \emph{higher} behavioral variance under filler (CV = 0.132 vs.\ 0.112 for control). Only the commitment-specific convergence beyond this shared-text baseline corresponds to reduced behavioral variance. This closes the causal loop: the commitment prompt changes hidden-state geometry at the commitment juncture, and these representational changes correspond to the observed behavioral improvements.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/intervention_similarity_step_progression.pdf}
\caption{Activation similarity (layer~40) across agent steps for all three intervention conditions ($n{=}89$ matched questions). All conditions are identical through step~3 (intervention point, dashed line), then diverge at step~4: commitment produces the highest convergence (0.995), filler intermediate (0.979), and control lowest (0.922). The causal timing---no pre-existing differences, sharp divergence one step after intervention---confirms the prompt changes hidden-state geometry at the commitment juncture.}
...
\textbf{Question-level decomposition.} A 3$\times$3 transition matrix classifying each question under both conditions---consistent-correct (accuracy $\geq 0.8$), consistent-wrong (accuracy $\leq 0.2$, majority frequency $\geq 0.6$), and inconsistent---reveals stability: 88 of 89 consistent-correct questions remain consistent-correct under commitment, all 4 consistent-wrong questions remain consistent-wrong, and zero questions transition from consistent-correct to consistent-wrong.\footnote{Under corrected matching, the control condition yields 89 consistent-correct, 4 consistent-wrong, and 7 inconsistent questions. The higher consistent-correct count relative to the 64 reported in Section~4.4 (which uses original substring matching) reflects questions reclassified as correct by token-F1.} The commitment intervention never degrades already-correct performance. At the question level, CV reduction and accuracy change are negatively correlated ($r = -0.32$, $p = .001$; Figure~\ref{fig:cv_acc_scatter}, Appendix~\ref{app:intervention_decomposition}): questions where commitment most reduced behavioral variance showed slight accuracy decreases, confirming that commitment amplifies whatever trajectory the model converges on---correct or not.

\textbf{Compute allocation.} We tested whether the commitment signal from $k{=}2$ initial runs could enable more efficient compute allocation by selectively retrying uncommitted questions. Using hidden states from the 53 control questions for which all 10 runs had complete step-4 hidden-state extraction (the remaining 47 had at least one run terminating before step~4 or incomplete extraction), we compared commitment-aware allocation against fixed-budget majority voting ($k_\text{init} \in \{2, 3\}$, $k_\text{extra} \in \{3, 5, 8\}$, 500 bootstrap iterations). The commitment-aware strategy does not meaningfully outperform fixed majority voting at matched compute (93.4\% vs.\ 93.1\% at ${\sim}$3 runs/question). While the full $k{=}10$ signal achieves strong classification (AUROC 0.97; Section~4.7), the $k{=}2$ signal is too noisy for reliable per-question routing---the commitment signal's primary value is \emph{diagnostic}: identifying which questions are likely to produce inconsistent behavior, rather than directly improving accuracy through compute routing.
...
\textbf{Representational commitment as a new axis.} Recent work has identified linear directions encoding persona \citep{anthropic_persona}, default behavior \citep{anthropic_axis}, and truth \citep{marks2024geometry}. Commitment---the degree to which a model's representations converge across runs to a stable interpretation---may constitute another such axis, but unlike these static properties, it emerges \emph{dynamically} during multi-step interaction and resolves at a specific \emph{commitment juncture}. This makes it specifically relevant to agentic settings: it indexes whether the model has arrived at a robust internal state during a task, not a property of its weights.

\textbf{From measurement to reliability.} Behavioral consistency predicts correctness: \citet{anonymous2026a} showed that selective prediction via unanimous agreement improves accuracy by 6--14pp at 54--62\% coverage in agentic settings, while \citet{anonymous2026b} revealed that consistency amplifies outcomes in both directions. The present work provides the representational mechanism underlying these behavioral findings: the commitment signal in hidden states explains \emph{why} some questions produce consistent behavior and others do not. The consistency predictor (AUROC 0.97) demonstrates this signal is detectable from internal states before a trajectory completes, offering a pathway to selective prediction that does not require multiple full runs. The prompting intervention provides causal evidence at both the behavioral and representational levels: the commitment prompt changes hidden-state similarity at the commitment juncture (Section~\ref{sec:intervention}), and this representational change corresponds to the observed behavioral improvement. The question-level decomposition shows commitment amplifies whatever trajectory the model is on---never degrading correct performance (zero consistent-correct $\to$ consistent-wrong transitions). The commitment signal's value is diagnostic: identifying which inputs will produce reliable behavior, enabling deployment strategies that allocate additional compute or human review where the model has not committed.

\textbf{Why step~4?} By step~4, the model has received 3--4 observations. Models whose representations converge across runs at this stage---arriving at a stable interpretation regardless of which specific evidence they received---proceed consistently; those whose representations remain sensitive to the specific evidence path diverge. The signal fading at step~5 suggests trajectories have already committed or scattered.
...
We introduced \emph{representational commitment}---the degree to which hidden states converge across runs to a stable interpretation---and showed it predicts behavioral consistency in LLM agents ($r = -0.35$, partial $r = -0.45$ after difficulty controls, 4.1$\times$ quartile effect). The signal predicts consistency but not correctness, is not explained by simple baselines, and generalizes across three architectures (Llama 70B, Qwen 72B, Phi-3 14B) and two benchmarks (HotpotQA, StrategyQA). A consistency predictor achieves AUROC up to 0.97, and a three-condition intervention confirms that commitment framing---not token count---reduces variance by 28\% ($d = 0.33$, $p = .001$) while simultaneously increasing hidden-state convergence at the commitment juncture ($d = 0.97$), closing the causal loop from prompt to representation to behavior. These findings demonstrate that representational commitment is both a measurable phenomenon and a diagnostically informative signal for agent reliability---identifying which inputs will produce consistent behavior and which require additional verification.
...
\caption{Question-level relationship between CV reduction ($x$) and accuracy change ($y$) under the commitment intervention ($n{=}100$). The negative correlation ($r = -0.32$, $p = .001$) shows that commitment amplifies the model's existing trajectory---reducing behavioral variance without systematically improving or degrading correctness.}
