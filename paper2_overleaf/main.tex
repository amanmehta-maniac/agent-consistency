%%%%%%%% ICML 2026 PAPER - 3-MODEL VERSION %%%%%%%%%%%%%%%%%
...
\icmltitlerunning{Consistency Amplifies: Behavioral Variance in LLM Agents}

\begin{document}

\twocolumn[
\icmltitle{Consistency Amplifies: How Behavioral Variance \\
           Shapes Agent Accuracy on SWE-bench}
...
\begin{abstract}
As LLM-based agents are deployed in production systems, understanding their behavioral consistency (whether they produce similar action sequences when given identical tasks) becomes critical for reliability. We study consistency in the context of SWE-bench, a challenging software engineering benchmark requiring complex, multi-step reasoning. Comparing Claude 4.5 Sonnet, GPT-5, and Llama-3.1-70B across 50 runs each (10 tasks $\times$ 5 runs), we find that consistency strongly predicts accuracy across models: Claude achieves the lowest variance (CV: 13.6\%) and highest accuracy (58\%), GPT-5 is intermediate (CV: 32.2\%, accuracy: 32\%), and Llama shows the highest variance (CV: 42.0\%) with lowest accuracy (4\%). However, our analysis reveals a critical nuance: \textbf{consistency amplifies outcomes rather than guaranteeing correctness}. 71\% of Claude's failures stem from ``consistent wrong interpretation'': making the same incorrect assumption across all runs. Interestingly, GPT-5 achieves similar early strategic agreement as Claude (diverging at step 3.4 vs 3.2) but exhibits 2.4$\times$ higher variance, suggesting that divergence timing alone does not determine consistency. These findings suggest that for production deployment, interpretation accuracy matters more than execution consistency, with implications for agent evaluation and training.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{Quantitative characterization across capability tiers:} We find that consistency strongly predicts accuracy across three models: Claude 4.5 Sonnet (CV: 13.6\%, accuracy: 58\%), GPT-5 (CV: 32.2\%, accuracy: 32\%), and Llama-3.1-70B (CV: 42.0\%, accuracy: 4\%).
    ...
    \item \textbf{The speed-accuracy-consistency tradeoff:} GPT-5 is 4.7$\times$ faster than Claude (9.9 vs 46.1 steps) but achieves 1.8$\times$ lower accuracy and 2.4$\times$ worse consistency, revealing a fundamental tradeoff.
    ...
    \item \textbf{Divergence timing vs consistency:} Despite similar divergence timing (Claude: step 3.2, GPT-5: step 3.4), Claude achieves 2.4$\times$ better consistency, showing that early agreement alone does not determine behavioral variance.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{GPT-5} (\texttt{openai-gpt-5}): OpenAI's frontier model with strong reasoning capabilities.
...
This yields 50 runs per model (10 tasks $\times$ 5 runs), totaling 150 agent trajectories across three models. All runs completed without infrastructure failures.
...
\midrule
Consistency (CV) & \textbf{13.6\%} & 32.2\% & 42.0\% \\
...
The three models show a clear hierarchy. Claude is significantly more consistent than GPT-5 ($t(18) = 2.59$, $p = 0.018$, Cohen's $d = 1.16$), which trends more consistent than Llama ($t(18) = -1.92$, $p = 0.071$). Claude's CV of 13.6\% indicates step counts vary by about 6 steps around a mean of 46, while GPT-5's 32.2\% CV indicates variation of about 3 steps around a mean of 10.
...
\paragraph{The Speed-Accuracy-Consistency Tradeoff.} GPT-5 reveals an important tradeoff: it is 4.7$\times$ faster than Claude (9.9 vs 46.1 steps) but achieves 1.8$\times$ lower accuracy (32\% vs 58\%) and 2.4$\times$ worse consistency (CV: 32.2\% vs 13.6\%). This suggests that thoroughness trades off against speed, with consistency as a mediating factor.
...
\begin{table}[t]
\caption{Divergence analysis: Claude and GPT-5 diverge at similar steps, but Claude achieves 2.4$\times$ better consistency.}
...
\begin{itemize}
    ...
    \item 2.4$\times$ worse consistency (CV: 32.2\% vs 13.6\%)
\end{itemize}
...
We studied behavioral consistency in LLM agents on SWE-bench across three models, finding a clear hierarchy: Claude achieves the highest consistency (CV: 13.6\%) and accuracy (58\%), GPT-5 is intermediate (CV: 32.2\%, accuracy: 32\%), and Llama shows the highest variance (CV: 42.0\%) with lowest accuracy (4\%).
...
GPT-5 reveals a speed-accuracy-consistency tradeoff: it is 4.7$\times$ faster than Claude but achieves 1.8$\times$ lower accuracy and 2.4$\times$ worse consistency. These findings suggest that interpretation quality, not execution consistency, is the primary bottleneck for reliable agent deployment. Future work should explore adaptive strategies that balance thoroughness with efficiency based on task complexity.
...
\midrule
12907 & 18.5 & 5/5 & 45 & 30.8 & 2/5 & 11 & 59.6 & 0/5 & 18 \\
13033 & 12.7 & 3/5 & 48 & 66.7 & 0/5 & 9 & 39.7 & 0/5 & 16 \\
13236 & 24.2 & 0/5 & 41 & 35.0 & 0/5 & 7 & 34.7 & 1/5 & 15 \\
13398 & 7.9 & 0/5 & 49 & 20.7 & 0/5 & 9 & 42.0 & 0/5 & 18 \\
13453 & 12.8 & 5/5 & 45 & 39.2 & 3/5 & 17 & 25.7 & 0/5 & 16 \\
13579 & 9.4 & 5/5 & 49 & 13.6 & 3/5 & 8 & 42.5 & 0/5 & 18 \\
13977 & 13.1 & 0/5 & 46 & 0.0 & 0/5 & 7 & 33.7 & 0/5 & 18 \\
14096 & 15.5 & 5/5 & 41 & 55.6 & 3/5 & 16 & 27.8 & 0/5 & 14 \\
14182 & 11.1 & 1/5 & 48 & 18.1 & 0/5 & 7 & 54.6 & 0/5 & 24 \\
14309 & 10.7 & 5/5 & 49 & 42.1 & 5/5 & 7 & 60.1 & 1/5 & 13 \\
\midrule
\textbf{Mean} & \textbf{13.6} & \textbf{29/50} & \textbf{46.1} & \textbf{32.2} & \textbf{16/50} & \textbf{9.9} & \textbf{42.0} & \textbf{2/50} & \textbf{17.0} \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
\paragraph{CV Comparisons.} Pairwise t-tests comparing per-task CV:
\begin{itemize}
    \item GPT-5 vs Claude: $t(18) = 2.59$, $p = 0.018$, $d = 1.16$ (significant)
    \item GPT-5 vs Llama: $t(18) = -1.92$, $p = 0.071$, $d = -0.86$ (not significant)
    \item Claude vs Llama: $t(18) = -6.75$, $p < 0.001$, $d = -3.02$ (significant)
\end{itemize}

\paragraph{Accuracy Comparisons.} Fisher's exact test:
\begin{itemize}
...
\paragraph{Divergence Comparisons.} Pairwise t-tests:
\begin{itemize}
    \item GPT-5 vs Claude: $t(18) = 0.25$, $p = 0.803$ (not significant)
    \item GPT-5 vs Llama: $t(18) = 5.77$, $p < 0.001$ (significant)
    \item Claude vs Llama: $t(18) = 2.57$, $p = 0.019$ (significant)
\end{itemize}
